...
switch loc_type
    case 'inout'
        bbox_pred_in_region_quantized = decode_in_out_probabilities(loc_prob_vectors, class_indices_inptut);
    case 'borders'
        bbox_pred_in_region_quantized = decode_borders_probabilities(loc_prob_vectors, class_indices_inptut);    
    case 'combined'
        bbox_pred_in_region_quantized = decode_combined_probabilities(loc_prob_vectors, class_indices_inptut);
    otherwise
...
real_coord(:,1) = real_coord_region(:,1)+bbox_search_region(:,1) - 1;
real_coord(:,3) = real_coord_region(:,3)+bbox_search_region(:,1) - 1;
real_coord(:,2) = real_coord_region(:,2)+bbox_search_region(:,2) - 1;
real_coord(:,4) = real_coord_region(:,4)+bbox_search_region(:,2) - 1;
end
...
assert(max(class_indices) <= size(loc_maps_per_cls,3));
loc_maps_size(3) = 1;
