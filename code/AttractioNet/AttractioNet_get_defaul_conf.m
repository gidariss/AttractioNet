...
ip.addParameter('num_iterations',         5,  @isscalar);  % number of iterations.
ip.addParameter('num_seed_boxes',     10000,  @isscalar);  % number of seed boxes.
ip.addParameter('max_per_image_init',  2000,  @isscalar);  % number of boxes that will be kept after the first iteration.
ip.addParameter('iou_thrs_close',      0.90,  @isscalar);  
...
ip.addParameter('num_output_boxes',    2000,  @isscalar);  
ip.addParameter('scales',             1000,   @isnumeric);  % the shortest dimension of the image before is fed to the proposal model
ip.addParameter('max_size',           1400,   @isscalar); % maximum size of the lon
